...
The code below analyzes the dataframe \texttt{movies} in the \texttt{ggplot2movies} package, which contains 24 attributes (genre, year, budget, user ratings, etc.) for 58788 movies obtained from the website \href{http://www.imdb.com}{http://www.imdb.com} with some missing values. Type \texttt{help(movies)} after loading the package \texttt{ggplot2movies} for more information. 
...
\item[Winsorization.] Shrink outliers to border of main part of data. One special case of this is to replace outliers with the most extreme of the remaining values.
