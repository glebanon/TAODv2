...
A relational database management system (RDBMS) is a computer program that simplify the process of storing and retrieving data, in particular when the data is large and there are several users storing and retrieving data. Relational databases typically provide the following capabilities.
\begin{enumerate}
\item Relational databases allow users to create a new database and define its schema, which is a specification of the type of data that can reside in the database.	
\item Relational databases let users query the database using a query language called structured query language (SQL). The RDBMS process the query and returns to the user the corresponding portion of the data (or sometimes a computation based on the data).
