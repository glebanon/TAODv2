...
\subsection{Git} \label{chap:tools:git}
...
The command \texttt{git reset --hard} resets the working directory to the last previous commit. This is useful if you made some changes that you regret but did not commit them yet.
...
Appending a branch name (including \texttt{master}) with \~{}\texttt{k} creates a ref to the commit that is \texttt{k} commits before the commit at the top of the branch. Appending to the branch name \^{} corresponds to \~{}1 and \^{}\^{} corresponds to \~{}2 (for example \texttt{master\^{}\^{}} is equivalent to \texttt{master}\~{}\texttt{2}). 
...
Another possibility to refer to commits is using tags. We can create a user specifie tag to annotate the current commit using the command \texttt{git tag tag\_name}.
...
We can also revert a bad commit using the command \texttt{git revert ref\_name}. y666g
...
There are two basic ways of helping Git in resolving merge conflict. The first is to edit the files with the markers above, remove the markers and make sure the files contain the appropriately merged version. After the conflicts are resolved manually by editing, we need to a perform a \texttt{git add file\_name} and \texttt{git commit} commit to inform Git that we resolved the conflict. The second way to resolve conflicts is to use a graphic merge tool by running the command \texttt{git mergetool}. This command starts a graphic merge tool such as \texttt{opendiff} or \texttt{kdiff3}. The precise tool depends on the operating system and \texttt{git mergetool} needs to be configured appropriately first (see \texttt{git help mergetool} for help on configuring the mergetool).

\subsubsection{Remote Git}

In many cases developers need to work with Git respositories on other servers. In this case, the first stage is to clone the respository on a local computer using the \texttt{git clone respository} command where \texttt{respository} has a pattern such as \texttt{ssh://username@address.com/path/projectName.git}. For example the following command creates a cloned repository of the \texttt{projectName.git} repository in the local \texttt{projectName} directory. The repository that was cloned is referred to as \texttt{origin} and the cloned repository is referred to as \texttt{remote}. 

<<eval=FALSE>>=
git clone ssh://username@address.com/path/projectName.git
@

At this point the developer can move into the newly created directory and start editing files, adding files, and committing as outlined in the previous section. One major difference is that the commit will be stored in the local repository (\texttt{remote}) index rather than the repository that was cloned (\texttt{origin}). To propagate the commits to \texttt{origin} we need to execute the \texttt{git push} command. 

The \texttt{git pull} command contacts \texttt{origin} and checks for recent updates. If there are updates on \texttt{origin}, they are downloaded to the local directory and merged with the working version (if conflicts occur they need to be resolved manually as described above). 
...
GitHub is a Git service hosted in the cloud that provides Git functionality as well as a fea additional features. Many open source software projects use GitHub to host their source code and development efforts. Some companies (for example Netflix) also use use GitHub as their source code and code development respository.

It is easy to start a free account on GitHub (visit \href{http://www.github.com}{http://www.github.com}) and start a Git respository. Afterwards you can clone your GitHub repository and create a local repository. At that point you can issue the standard Git commands to commit changes locally, and the push and pull so that the local repository (remote) synchronizes with the original repository (origin). Alternatively, GitHub offers a web interface through \href{http://www.github.com}{http://www.github.com} where you can execute many GitHub commands instead of using the command line.
...
GitHub hosts many open source projects whose code is meant to be disseminated widely, but only a small team of developers are meant to modify the code. GitHub's fork operation addresses this issue. It is similar to clone, but rather than downloading the entire repository it creates a clone on GitHub's servers that is a separate repository. The user can create a local version, make changes to it and do commits, but any pushes will modify the hosted copy and not the original repository. This facilitates open source collaboration process where a few people have permission to modify the main source code project, but everyone have access to download the latest version (or a development branch) of the software as well as create their own version (fork) and modify it.

Pull-request allows people without permission to modify the original (forked) repository submit requests to people with permission to modify the original repository. After commiting a change to the forked repository, a pull-request send a request to one or more people with commit access to the original repository. These people are notified of the reuqest and they can view the commit and its diff. They can then approve the request and merge the commit into the main repository, or can leave comments outlining objections to the new version or requests for modifications. If a request for modifications are made the person who made the original commit can make modifications after which the new commit is sent again for review.
...
Executing a fork or a pull request can be made through the web interface at \href{http://www.github.com}{http://www.github.com} or through a command line tool \texttt{gh} using \texttt{gh fork} and \texttt{gh pull-request}. The website \href{http://www.github.com}{http://www.github.com} contains tutorials explaining how to use fork and pull-requests in detail.

\subsection{Subversion} 

Subversion is an older version control system that is a successor to the older version control system  Concurrent Versions System (CVS). It is still widely used today in many open source and industry projects. In contrast to Git, it is centralized, rather than distributed. The working copy that is stored on the client does not hold the entire repository information. Instead is stores the recent commit and the edits made on top of it. 

To check out a working copy of a repsository type the command \texttt{svn checkout repos-address} where \texttt{repos-address} specifies the address of the repository, for example \texttt{http://server.com/svn/project1} or \\ \texttt{svn+ssh://username@server.com/svn/project1}. 

To update the local version with changes that were made to the repository we use the command \texttt{svn update} (analogous to \texttt{git pull}). This performs a merge if needed, including manual resolution of merge conflicts if they exist (similar to the Git merge process). 

Since the working copy in Subversion is not a repository (as in the case of Git), the commit command \texttt{svn commit -m "commit message"} updates the repository. It is analogous to \texttt{git commit} followed by \texttt{git push}. It is necessary to run \texttt{svn update} before \texttt{svn commit} if there are new updates on the remote repository. 

Several other Subversion commands are similar to Git: \texttt{svn add} to add files, \texttt{svn cp} to copy files, and  \texttt{svn mv} to move files. Also, Subversion has its own diff (\texttt{svn diff}), log (\texttt{svn log}), status (\texttt{svn status}), and revert (\texttt{svn revert}) commands. These commands are similar to the Git counterparts described in Section~\ref{chap:tools:git}. 

In contrast to Git's SHA-1 commit codenames, Subversion uses sequential numbers to denote versions. This is not a problem since Subversion is centralized and can thus enforce a clear numeric structure on the commits of different contributors. 
Switching to an earlier version in Subversion can be done by a reverse merge: for example if the repository is at version 10 the command \texttt{svn merge -r 10:9} will bring back version 9 to the working copy. A subsequent \texttt{svn commit} will commit this change to the repository. 

Subversion's implements branches by copying files into a new directory representing the new branch. For example, the command below copies the repository trunk (representing stable version) into a new directory that can host feature work. 
...
This updates the working copy. Executing \texttt{svn commit} will update the repository.

Git has several advantages over Subversion, including: (a) not requiring network access for many operations, (b) faster commands (due to no need for network latency in some cases), (c) considerably smaller repository files, and (d) more lightweight implementation of branches. As a result Git has been gaining consderable popularity recently.


%-----------------------------------
\section{Build Tools} \label{sec:tools:ant}
 
Complex software requires a complex build process: (a) compiling files that were changed since the last compilation, (b) linking different files together and possibly with external libraries, (c) running tests to diagnose potential problems with the code. Build tools enable automating this process so that less manual effort is needed and fewer mistakes are made. It is an essential part of almost all large scale software development processes.

At the heart of build tools is declaring a list of tasks that need to be executed, specifying the dependency between tasks and files and between different tasks, and providing implementation details for executing the tasks. These details are expressed in one or more build files that are written in a specific format depending on the build tool that is used. The build tool reads the build file or files and executes the appropriate tasks in order. 
...
The make program executes a sequence of instruction specified in a text file called \texttt{makefile} or \texttt{Makefile}. Specifically, running the command \texttt{make target\_name} in the shell prompt executes the sequence of instructions (specified in the \texttt{makefile}) needed to build the target \texttt{target\_name}.

The \texttt{makefile} file is a text file containing a list of instructions for building one or more targets. Each target has the following format:
...
Above, \texttt{target} is a label describing the task and \texttt{prereq\_1} to \texttt{prereq\_k} specify files that the target depends on. In other words, if the prerequisite files change since the previous make execution, the make program needs to rebuild the target. The commands \texttt{command\_1} to \texttt{command\_m} are prefixed by a tab and describe the process that builds the specified target. In general there may be zero or more prerequisite files and zero or more commands in each target. Text following \texttt{\#} are interpreted as comments.

The above syntax for building a target is typically repeated multiple times in the makefile, usually with more general targets appearing at the top of the file and including more specific targets as prerequisites. Running \texttt{make} without a target argument builds the first target.

Prerequisite in the makefile can be specified using wildcards (for example \texttt{*.cpp} expands to all files ending with \texttt{.cpp} in the current directory). Make can also use variables,  denoted by uppercase labels, in the specification of the commands. There are several core variables including: \texttt{\$\@} -- the filename representing the current target, and \texttt{\$<} -- the filename of the first prerequisite. Custom variables can also be defined in the makefile (for example \texttt{CC=gcc}) and referred to afterwards using the \texttt{\$} symbol (for example \texttt{\$(CC)}).

Make also has pattern rules that can significantly simplify the makefile structure. A pattern rule is similar to a normal rule except that the stem of the file (filename without suffix) is represented by the \texttt{\%} character. For example, the rule below can be thought of as  multiple rules matching each .o file with a target whose prerequisite file is the corresponding .cpp file and the whose command compiles (without linking due to the \texttt{-c flag}) the .cpp file into the object file whose name is specified by the target.
...
As our first example, lets consider a ``hello world'' program in C++ that includes three files. The first file represents the main executable function and is called \texttt{hello.cpp}.
...
The second file contains the implementation of the \texttt{helloWorld} function.
...
The third file contains the .h header file of the \texttt{helloWorld} function.

<<eval=FALSE>>=
void helloWorld(void);
@

The normal compilation command is as follows. It instructs the compiler to compile \texttt{hellomake.c} and \texttt{hellofunc.c}, name the executable file \texttt{hello}, and look at the current directory for any header files included in the specified source files. 
...
# command compiles (without linking) source file into object files
%.o: %.cpp $(DEPS)
	$(CC) -c -o $@ $< $(CPPFLAGS)
...
The first rule above represents specifies that the \texttt{hello} target depends on the two object files (compiled but not linked) and specifies the compilation command using the custom variables \texttt{CPPC} (compilation command) and \texttt{CPPFLAGS} (flags). 

The second rule specifies that each .o object file depends on the curresponding .cpp file and the include file. The command compiles the .cpp file (using the variable \texttt{\$<}) into the .o file (using the variable \texttt{\$@}) (the \texttt{-o} flag specifies compilation without linking). 

The third rule cleans up the directory removing intermediate files and leaving only the original source files and the final executable.

When we run \texttt{make hello}, the make program checks if any of the .o pre-requisites have changed and require rebuilding. This is done by checking the second target that depends on the corresponding .cpp file and the header file. If the .cpp and the .h files did not change nothing is compiled. If only one of the .cpp files changed the second rule is applied for that file followed by the first rule. Generalizing this simple makefile to a larger more complex program can automate the build process without running unnecessary compilation stages. 

There are two main principles for writing good makefiles: (a) separating compilation steps as much as possible, and using variables to separate logic from names and directory choices. We saw above how the first principle above can lead to significantly reduced compilation time for large projects. The second principle is useful in that revising the program (for example by deciding to move the header files to a different directory or by adding additional .cpp files) require a simple modification to the definitions of the custom variables rather than rewriting the build commands.  
...
A third difference is that Ant provides a nice integration with Java unit tests. As a result, Ant conveniently integrates unit tests into the build process and is particularly convenient for test-driven development process. We postpone a description of testing and integrating testing into build tools to Section~\ref{tools:testing}.

We describe below several important aspects of the Ant XML file format, following by an example build file and some final remarks.
...
XML elements in general have the following format: \texttt{<tag\_type attr\_1=val\_1 ... attr\_k=val\_k> ... <tag\_type/>} or \texttt{<tag\_type attr\_1=val\_1 ... attr\_k=val\_k/>}. The first option is used to host additional nested elements inside the outer element denoted above using the \texttt{...} notation. The second option is used for elements that do not have nested elements inside of them. In the XML example above the project element is the outer element and has the attribute \texttt{name} and \texttt{default} that get the values \texttt{"hello"} and \texttt{"build"} respectively. The project name attribute is used to refer to the project and the default attribute refers to the default target tasks. 

The target element has a \texttt{name} attribute and optionally a \texttt{depends} attribute hosting the list of targets that it depends on (for example in the build file above the \texttt{deploy} target depends on \texttt{build} target and \texttt{test} target) and a \texttt{description} attribute that documents the target using a short readable comment. 

When the command \texttt{ant target\_name} is executed in the shell, Ant looks for a build.xml file in the current directory and then matches \texttt{target\_name} with one of the target elements in the build file. Ant then executes the dependencies of that target followed by the instructions inside the target element. If we execute the command \texttt{ant} without a target name, Ant executes the target marked as default in the project element (for example \texttt{build} target in the example above).

The instructions for executing the targets are usually given using XML elements such as \texttt{<delete>} (deletes files), \texttt{<javac>} (Java compiler), \texttt{<mkdir>} (creating a directory). These XML elements and others map to executable commands that are platform independent (for example the \texttt{<delete>} element will map to the right shell command for removing files based on the operating system). 

Below is a simple Ant file for building and cleaning a simple hello world Java application that contains a single file. Note that presence of the two targets: \texttt{build} and \texttt{clean}, the first of which is the default target. The first target creates a directory for the class file and runs the Java compiler on the specfiied directory. The \texttt{javac} element a \texttt{src} element that describes the location of the source files and several additional optional attributes that are passed to the Java compiler. The \texttt{delete} element has a \texttt{fileset} element that describes the set of files to act one. The \texttt{fileset} tag has a directory attribute and an optional nested \texttt{include} element that filters the files to act on using for file name patterns (for example \texttt{**/*.java} refers to all files with a \texttt{.java} file extension that lie in the current directory or one of its subdirectories).
...
Putting a Hello World Java application (for example the file below) in the subdirectory \texttt{src/} and running \texttt{ant} will build the class file using the Java compiler and store it in the \texttt{classes/} subdirectory.
...
Ant can define variables (for example for storing directory paths) using the XML element \texttt{property}. Predefined properties include for example \texttt{basedir} (base diretory), \texttt{ant.file} (location of the build file), and \texttt{ant.java.version} (version of JDK used by Ant). Additional properties can be defined by specifying a corresponding XML element (see example below).

<<eval=FALSE>>=
<property name="ProjectURL" value="www.theanalysisofdata.com">
@

To refer to properties in the XML file we enclose the property name with the following structure \texttt{\$\{...\}}. Properties can also be specified in an external file, in which case they are listed as a list of \texttt{property\_name=property\_val} (the name of the external file needs to be specified in the build file using the following element: \texttt{property file="filename"/>}). This allows using switching between different property files without modifying the XML build file. 

\subsection{Gradle}

Gradle is a recent build tools that has several advantages over Ant. It is suited for a large number of languages (in addition to Java) and its build files are more readable and concise than Ant's XML file or Makefiles. Specifically, Gradle build files are specied used Groovy language - a dynamic programming language similar to Python that is dynamically compiled to run on the Java Virtual Machine. 
 Gradle is gaining in popularity in recent years and is integrated with most popular IDEs (for example Eclipse or IntelliJ).
...
By deafult, Gradle's Java plugin assumes the following directory structure, borrowed from Maven, another popular Java build tool. 

\begin{description}
\item[\texttt{.}]: The base directory of the project containing the following subdirectory and the build file \texttt{build.gradle}.
\item[\texttt{src/main/java}]: Contains the Java production source files.
\item[\texttt{src/main/resources}]: Contains additional project files (for example additional JAR files)
\item[\texttt{src/test/src}]: Contains the Java test source files
\item[\texttt{src/test/resources}]: Contains additional test files (for example additional JAR files)
\item[\texttt{build}]: Contains the output compilation files
\end{description}

In contrast to Ant and Make, Gradle can proceed with a nearly empty build file assuming we use the directory structure above. As a simple example consider copying the hello world Java file described in Section~\ref{sec:tools:ant} to \texttt{src/main/java} and having a nearly empty build file \texttt{build.gradle} instructing gradle to use the Java plugin as follows.

<<eval=FALSE>>=
apply plugin: 'java'
@

When we execute the command \texttt{gradle build}, Gradle will compile the source file and put the compiled class file in \texttt{build/classes/main}. The command \texttt{gradle clean} will remove compiled files in the \texttt{build} directory and \texttt{gradle test} will compile the source code and run tests specifying in the \texttt{src/test} directory. Gradle defines automatically these targets or tasks (build, clean, check), as well as other targets such as assemble, test, and jar. The implementations are already defined for these tasks as well as the dependencies (for example the build target depends on the assembly target as well as the check target that in turns depends on the test target).

Gradle can adapt to non-conventional directory structures by assigning values to the appropriate variables. For example, the following build.gradle file instructs gradle to use a custom definition of the source file directory.
...
Gradle can be used as in the examples above, where the definition of the directory structure, target implementations, dependencies are prespecified. In this case, writing the build file is very easy and fast and it provides significant functionality.  Gradle can also be used when customizing some or many of these conventions (directory structure, target implementations, dependencies), in which case it offers significant flexibility at the cost of a need to carefully author the build file. In any case, Gradle's build files are more readable than Makefiles or Ant's XML build files and are thus well suited for large complex projects.
...
%-----------------------------------
\section{Software Tests} \label{tools:testing}
